...
function create_graq(total_entradas, total_salidas) {
    ...
    tt_entradas = (parseFloat(total_entradas * 100) / tt).toFixed(2);
    tt_salidas = (parseFloat(total_salidas * 100) / tt).toFixed(2)
    if (isNaN(tt_entradas)) {
        $('#contiene_chart').css('display', 'none')
        tt_entradas = '0'
    } else {
        $('#contiene_chart').css('display', 'block')
        tt_entradas = tt_entradas
    }
    if (isNaN(tt_salidas)) {
        $('#contiene_chart').css('display', 'none')
        tt_salidas = '0'
        ...
        $('#contiene_chart').css('display', 'block')
        tt_salidas = tt_salidas
    }
    document.querySelector("#porcentaje_entradas").textContent = "Entradas: " + tt_entradas + "%";
    document.querySelector("#porcentaje_salidas").textContent = "Salidas: " + tt_salidas + "%";
